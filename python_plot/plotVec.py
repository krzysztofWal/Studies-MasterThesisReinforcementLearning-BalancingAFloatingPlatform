...
def update(ind):
    ...
    title.set_text("t = {:.2f}".format(ind/200))
    quiver.remove()
    #quiver_r1.remove()
    quiver = ax.quiver(*get_arrow('tmp',ind, scaling_factor), arrow_length_ratio=0.1)
    #quiver_r1 = ax.quiver(*get_arrow('r1',ind, 1), arrow_length_ratio=0.1,colors=r1_c)

ani = FuncAnimation(fig, update, frames=np.linspace(0,1000, 50), interval=10, repeat=True)
plt.show()
